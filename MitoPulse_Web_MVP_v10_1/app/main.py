...
DATABASE_URL = os.getenv("DATABASE_URL", "sqlite:///./mitopulse_v101.db")
...
app = FastAPI(title="MitoPulse Web MVP v10.1")
...
async def api_register_node(payload: dict):
    ...
    db.commit()
    out = {"node_id": node.id, "user_id": node.user_id, "platform": node.platform, "pulse_count": node.pulse_count or 0, "cross_pulse_count": node.cross_pulse_count or 0, "last_seen_at": node.last_seen_at, "status": node.status}
    db.close()
    await manager.broadcast({"type": "refresh"})
...
async def api_pulse(payload: dict):
    ...
    db.commit()
    out = {"node_id": node.id, "pulse_count": node.pulse_count, "last_seen_at": node.last_seen_at, "status": node.status}
    db.close()
    await manager.broadcast({"type": "refresh"})
    return JSONResponse(out)
