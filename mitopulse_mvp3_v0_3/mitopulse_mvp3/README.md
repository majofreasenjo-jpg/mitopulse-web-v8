...
# (Opcional) si estás probando con datos históricos:
# export MITOPULSE_ALLOWED_SKEW_SECONDS=86400
# (Opcional) hardening:
# export MITOPULSE_API_KEY="change-me"  # exige header X-API-Key en /v1/*
# export MITOPULSE_RATE_LIMIT_RPS=10
...
- (Opcional) API key por header `X-API-Key` si defines `MITOPULSE_API_KEY`.
- (Opcional) Rate limiting simple por IP (`MITOPULSE_RATE_LIMIT_RPS`).
