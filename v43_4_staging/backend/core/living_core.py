...
def filter_entities(entities, tenant_id=None):
    if not tenant_id:
        return entities
    return [e for e in entities if e.get("tenant_id") == tenant_id]

def compute_kpis(entities):
    vals = [e.get("value", 0) for e in entities] or [0]
...
def build_graph(entities):
    ...
    return {
        "nodes": nodes,
        "links": links,
        "dynamic_waves": [{"entity": n["id"], "x": n["x"], "y": n["y"], "intensity": round(n["score"]/100,2)} for n in nodes if n["score"] >= 60],
