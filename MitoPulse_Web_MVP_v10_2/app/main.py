...
DATABASE_URL = os.getenv("DATABASE_URL", "sqlite:///./mitopulse_v102.db")
...
class CrossPulseSession(Base):
    __tablename__ = "cross_pulse_sessions"
    id = Column(String, primary_key=True)
    code = Column(String)
    node_a = Column(String)
    node_b = Column(String)
    status = Column(String, default="pending")
    shared_context_score = Column(Float, default=82.0)
    temporal_delta_ms = Column(Float, default=35.0)
    created_at = Column(Integer)
...
app = FastAPI(title="MitoPulse Web MVP v10.2")
...
def get_node_for_user(db, user_id: Optional[str]):
    if not user_id:
        return None
    return db.query(DeviceNode).filter(DeviceNode.user_id == user_id).order_by(DeviceNode.last_seen_at.desc()).first()
...
def evaluate(db, p: dict, source: str = "manual", expected_label: Optional[str] = None) -> dict:
    ...
    reasons = {
        "relational_distance_score": "No existe relación previa suficiente entre origen y destino." if rds < 20 else ("La relación existe, pero aún es débil." if rds < 55 else "Existe una relación razonablemente consistente entre los nodos."),
        "anchor_support_score": "El destino tiene poco soporte relacional o pocos guardianes cercanos." if anchor < 30 else ("El destino tiene soporte moderado en la red." if anchor < 70 else "El destino está respaldado por historial/guardianes de confianza."),
        "shared_reality": "No se detectó realidad compartida suficiente para este evento." if sr < 35 else ("Hay señales parciales de realidad compartida." if sr < 70 else "La interacción muestra alta realidad compartida o validación contextual."),
        "situational_coherence": "El evento es poco coherente con el contexto o la rutina declarada por la plataforma." if situ < 35 else ("El evento presenta cierta anomalía contextual." if situ < 70 else "El evento es situacionalmente coherente."),
        "fraud_exposure": "El destino o evento aparece cerca de patrones recientes de fraude en la red." if exposure >= 40 else ("Existe exposición media a riesgo propagado." if exposure >= 15 else "No hay exposición material a fraude propagado."),
        "fraud_hunter_score": "El nodo presenta comportamiento infeccioso o de campaña coordinada." if hunter >= 80 else ("El nodo muestra señales de riesgo y debe ser monitoreado." if hunter >= 35 else "No se observan señales fuertes de nodo infeccioso.")
    }
    ...
    elif decision == "QUARANTINE NODE":
        recommended_action = "quarantine_node"
    else:
        recommended_action = "step_up_authentication" if exposure < 10 and rds < 20 and sr < 35 else "hold_and_review"

    dominant_reason = max([
        ("relational_distance_score", 100 - rds),
        ("anchor_support_score", 100 - anchor),
        ("shared_reality", 100 - sr),
        ("situational_coherence", 100 - situ),
        ("fraud_exposure", exposure),
        ("fraud_hunter_score", hunter),
    ], key=lambda x: x[1])[0]

    audit_payload = {
        "formula": "TrustScore = 0.15*HP + 0.15*Freedom + 0.18*SharedReality + 0.17*SituationalCoherence + 0.15*RDS + 0.10*AnchorSupport + 0.10*ExternalReputation - 0.25*FraudExposure",
        "scenario": p.get("scenario", "bank_transfer"),
        ...
        "human_explanation": reasons,
        "context_summary": {
            "amount": p.get("amount", 0),
            "urgency": p.get("urgency", 0),
            "routine_break": p.get("routine_break", 0),
            "transaction_novelty": p.get("transaction_novelty", 0),
            "remote_capture_probability": p.get("remote_capture_probability", 0),
        },
        "top_reason": {
            "key": dominant_reason,
            "message": reasons.get(dominant_reason)
        }
    }
    audit_json = json.dumps(audit_payload, ensure_ascii=False)
    audit_receipt = make_id("audit")
    ...
    row = Evaluation(
        ...
        recommended_action=recommended_action, audit_json=audit_json, audit_receipt=audit_receipt,
    ...
    return {
        ...
        "recommended_action": recommended_action,
        "human_explanation": reasons,
        "top_reason": {"key": dominant_reason, "message": reasons.get(dominant_reason)},
...
def spread_map(db):
    ...
    for threat in threats[:8]:
        uid = threat["user_id"]
        for nb, weight in neighbors(db, uid):
            key = tuple(sorted([uid, nb]))
            if key in seen:
                continue
            seen.add(key)
            routes.append({
                ...
                "source_state": threat["state"],
                "source_score": threat["fraud_hunter_score"],
            })
    return {"threats": threats[:8], "routes": routes[:24]}
...
def api_nodes():
    ...
    out = []
    for r in rows:
        u = get_user(db, r.user_id) if r.user_id else None
        out.append({"id": r.id, "user_id": r.user_id, "platform": r.platform, "pulse_count": r.pulse_count, "cross_pulse_count": r.cross_pulse_count, "last_seen_at": r.last_seen_at, "is_guardian": bool(r.user_id in gids), "trust_rank": round((u.trust_rank if u else 0) or 0,2), "nodal_mass": round((u.nodal_mass if u else 0) or 0,2)})
    db.close()
    return out
...
def api_cross_pulses():
    ...
    out = [{"node_a": r.node_a, "node_b": r.node_b, "shared_context_score": r.shared_context_score, "temporal_delta_ms": r.temporal_delta_ms, "source": r.source, "created_at": r.created_at} for r in rows]
    db.close()
    return out
...
def api_recent_evaluations():
    ...
    out = [{
        ...
        "expected_label": r.expected_label, "created_at": r.created_at, "recommended_action": r.recommended_action,
        "top_reason": (json.loads(r.audit_json).get("top_reason") if r.audit_json else None)
    } for r in rows]
    db.close()
    return out
...
async def api_register_node(payload: dict):
    ...
    db.commit()
    u = get_user(db, node.user_id) if node.user_id else None
    out = {"node_id": node.id, "user_id": node.user_id, "platform": node.platform, "pulse_count": node.pulse_count or 0, "cross_pulse_count": node.cross_pulse_count or 0, "last_seen_at": node.last_seen_at, "status": node.status, "is_guardian": bool(node.user_id in guardian_ids(db)) if node.user_id else False, "trust_rank": round((u.trust_rank if u else 0) or 0,2), "nodal_mass": round((u.nodal_mass if u else 0) or 0,2)}
    db.close()
    await manager.broadcast({"type": "refresh"})
...
async def api_pulse(payload: dict):
    ...
    db.commit()
    u = get_user(db, node.user_id) if node.user_id else None
    out = {"node_id": node.id, "pulse_count": node.pulse_count, "last_seen_at": node.last_seen_at, "status": node.status, "trust_rank": round((u.trust_rank if u else 0) or 0,2), "nodal_mass": round((u.nodal_mass if u else 0) or 0,2)}
    db.close()
    await manager.broadcast({"type": "refresh"})
    return JSONResponse(out)
...
async def api_crosspulse(payload: dict):
    ...
    na, nb = get_node(db, a), get_node(db, b)
    before = None
    if na and nb and na.user_id and nb.user_id and na.user_id != nb.user_id and get_user(db, na.user_id) and get_user(db, nb.user_id):
        before = get_rel(db, na.user_id, nb.user_id).weight
    shared_context_score = float(payload.get("shared_context_score", 75))
    temporal_delta_ms = float(payload.get("temporal_delta_ms", 50))
    write_cross_pulse(db, a, b, shared_context_score, temporal_delta_ms, payload.get("source", "manual"))
    after = None
    if na and nb and na.user_id and nb.user_id and na.user_id != nb.user_id and get_user(db, na.user_id) and get_user(db, nb.user_id):
        after = get_rel(db, na.user_id, nb.user_id).weight
    db.commit()
    db.close()
    await manager.broadcast({"type": "refresh"})
    return JSONResponse({"ok": True, "node_a": a, "node_b": b, "shared_context_score": shared_context_score, "temporal_delta_ms": temporal_delta_ms, "relationship_weight_before": before, "relationship_weight_after": after})


@app.post("/api/crosspulse/session/create")
async def api_crosspulse_session_create(payload: dict):
    db = SessionLocal()
    node_a = payload.get("node_a")
    if not node_a or not get_node(db, node_a):
        db.close()
        return JSONResponse({"error": "unknown node_a"}, status_code=400)
    code = str(random.randint(100000, 999999))
    sess = CrossPulseSession(id=make_id("xps"), code=code, node_a=node_a, node_b=None, status="pending", shared_context_score=float(payload.get("shared_context_score", 84)), temporal_delta_ms=float(payload.get("temporal_delta_ms", 35)), created_at=now(), expires_at=now()+120, consumed_at=None)
    db.add(sess)
    db.commit()
    db.close()
    return JSONResponse({"session_id": sess.id, "code": code, "expires_in": 120})


@app.post("/api/crosspulse/session/confirm")
async def api_crosspulse_session_confirm(payload: dict):
    db = SessionLocal()
    code = str(payload.get("code", "")).strip()
    node_b = payload.get("node_b")
    sess = db.query(CrossPulseSession).filter(CrossPulseSession.code == code).order_by(CrossPulseSession.created_at.desc()).first()
    if not sess or sess.status != "pending" or sess.expires_at < now():
        db.close()
        return JSONResponse({"error": "session invalid or expired"}, status_code=400)
    if not node_b or not get_node(db, node_b) or node_b == sess.node_a:
        db.close()
        return JSONResponse({"error": "invalid node_b"}, status_code=400)
    na, nb = get_node(db, sess.node_a), get_node(db, node_b)
    before = None
    if na and nb and na.user_id and nb.user_id and na.user_id != nb.user_id and get_user(db, na.user_id) and get_user(db, nb.user_id):
        before = get_rel(db, na.user_id, nb.user_id).weight
    write_cross_pulse(db, sess.node_a, node_b, sess.shared_context_score, sess.temporal_delta_ms, "relay_handshake")
    sess.node_b = node_b
    ...
    db.close()
    await manager.broadcast({"type": "refresh"})
    return JSONResponse({"ok": True, "session_id": sess.id, "node_a": sess.node_a, "node_b": node_b, "shared_context_score": sess.shared_context_score, "temporal_delta_ms": sess.temporal_delta_ms, "relationship_weight_before": before, "relationship_weight_after": after})
...
def api_evaluation_audit(evaluation_id: str):
    db = SessionLocal()
    row = db.query(Evaluation).filter(Evaluation.id == evaluation_id).first()
    if not row:
        db.close()
        return JSONResponse({"error": "evaluation not found"}, status_code=404)
    payload = json.loads(row.audit_json) if row.audit_json else {}
    payload.update({
        "evaluation_id": row.id,
        "event_id": row.event_id,
