...
import time
app = FastAPI(title="MitoPulse Identity Engine", version="2.0.0")
...
def identity_evaluate(body: IdentityEval):
    STATE[(body.tenant_id, body.user_id, body.device_id)] = {
        "tenant_id": body.tenant_id, "user_id": body.user_id, "device_id": body.device_id,
        "epoch": body.epoch, "tier": body.tier, "index": body.index, "stability": body.stability,
        "stability_band": band(body.stability), "human_conf": body.human_conf,
        "risk": body.risk, "coercion": body.coercion, "context": body.context, "last_seen": int(time.time())
    }
