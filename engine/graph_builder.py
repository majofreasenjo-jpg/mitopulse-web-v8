...
def build_graph(customers, devices, events, signals):
    ...
    for _, row in customers.iterrows():
        G.add_node(row["customer_id"], node_type="customer", region=row.get("region", "na"), segment=row.get("segment", "na"))
    ...
    for _, row in devices.iterrows():
        ...
        G.add_edge(cust, dev, edge_type="owns_device", weight=0.25, label=row.get("risk_hint", "normal"))
